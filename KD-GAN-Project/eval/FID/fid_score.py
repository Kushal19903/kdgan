...
def get_activations(files, model, batch_size=50, dims=2048, cuda=False, verbose=False):
    ...
    Returns:
    -- A numpy array of dimension (num_images, dims) that contains the
...
def _compute_statistics_of_path(path, model, batch_size, dims, cuda):
    if path.endswith('.npz'):
        ...
        files = sorted([file for ext in ['jpg', 'jpeg', 'png', 'webp'] for file in path.glob('*.{}'.format(ext))])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path1', type=str, required=True,
                        help='Path to the first image folder or .npz statistic file')
    parser.add_argument('--path2', type=str, required=True,
                        help='Path to the second image folder or .npz statistic file')
